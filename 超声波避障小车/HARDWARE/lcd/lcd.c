...
{  							  
	...
	{   
		...
		for(t1=0;t1<8;t1++)
		{			    
			if(temp&0x80)LCD_Fast_DrawPoint(x,y,POINT_COLOR);//ASCII码为1画点(颜色为Ponitcolor)
			else if(mode==0)LCD_Fast_DrawPoint(x,y,BACK_COLOR);//如果非叠加方式为0，且ASCII码为0点为背景色
			temp<<=1;
			y++;
